...
def manhattan_sim_matrix(a, b, eps=1e-8):
    return manhattan_distance(a, b)
    
def elucidian_sim_matrix(a, b, eps=1e-8):
    return torch.cdist(a.contiguous(), b, p=2)
...
def euclidean_get_nn_avg_dist(fine_hidden, text_hidden, knn):
    ...
    # normalize_L2(fine_hidden)
    # normalize_L2(text_hidden)
    index = faiss.IndexFlatL2(fine_hidden.shape[1])
    index.add(fine_hidden)
    distances, ids = index.search(text_hidden, knn)
    # print(distances)
    return ids
...
def manhattan_get_nn_avg_dist(fine_hidden, text_hidden, knn):
    ...
    # normalize_L2(text_hidden)
    index = faiss.IndexFlatIP(fine_hidden.shape[1])
    index.set_distance_function(manhattan_distance)
    index.add(fine_hidden)
    distances, ids = index.search(text_hidden, knn)
    # print(distances)
    return ids
...
def manhattan_get_nn_avg_dist(fine_hidden, text_hidden, knn):
    ...
    distances = manhattan_distance(text_hidden, fine_hidden)
...
def manhattan_CSLS(fine_hidden, text_hidden):
    ...
    # batch_size, knn
    avg_text_knn_id = manhattan_get_nn_avg_dist(fine_hidden, text_hidden, knn)
    # batch_size * knn, 768
    top_label_hidden = torch.index_select(fine_hidden, 0, avg_text_knn_id.reshape(-1).cuda())
    # batch_size * batch_size, knn
...
class RobertaModelForLabelNCE(nn.Module):
    ...
    def forward(self, batched_data):
        ...
        # pdb.set_trace()
        if args.use_csls:
            logits_fine = manhattan_CSLS(fine_output, sequence_output)
            # pdb.set_trace()
            logits_coarse = manhattan_CSLS(coarse_output, sequence_output)
...
def criterion(text_hidden, fine_hidden, coarse_hidden, true_coarse, ground_fine, logits, target, indexs, coarse_logits, coarse_target, epoch=0, mode='train'):
    ...
    for i in range(batch_size):
        ...
        if args.use_csls:
            ...
            positive_dist = torch.cdist(text_hidden[i].contiguous(), true_labels, p=1)
            ...
            if args.use_hard:
                ...
                total_negative_sim = torch.cdist(text_hidden[i].contiguous(), total_negative_hidden, p=1)
                ...
                negative_dist = torch.cdist(text_hidden[i].contiguous(), false_labels, p=1)
        ...
        if args.use_coarse and not args.use_infonce:
            ...
            if args.use_csls:
                ...
                if args.use_hard:
                    ...
                    total_negative_coarses = torch.cdist(test_hidden[i], total_negative_hidden, p=1)
                    negative_coarse = torch.topk(total_negative_coarses, 1, largest=True).values
                else:
                    # negative_coarse = cosine_similarity(text_hidden[i].unsqueeze(0), false_coarse_label)
                    negative_coarse = torch.cdist(text_hidden[i].contiguous(), false_coarse_label, p=1)
                # positive_coarse = cosine_similarity(text_hidden[i].unsqueeze(0), true_coarse_label)
                positive_coarse = torch.cdist(text_hidden[i].contiguous(), true_coarse_label, p=1)
                # positive_fine = cosine_similarity(text_hidden[i].unsqueeze(0), true_labels)
                positive_fine = torch.cdist(text_hidden[i].contiguous(), true_labels, p=1)
...
def prediction(text_hidden, label_hidden, true_coarse):
    ...
    if args.prediction_mode == 'csls':
        total_sims = manhattan_CSLS(label_hidden, text_hidden)
    ...
    for i in range(batch_size):
        ...
        if args.prediction_mode == 'cos':
            ...
            # pdb.set_trace()
            sims = manhattan_distance(text_hidden[i].unsqueeze(0), true_labels)
        else:
